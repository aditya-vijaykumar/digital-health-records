...
public class DFindConsultations {
  ...
  public VBox display() {
    ...
    Text t1 = new Text("Patient's First Name");
    TextField patientFirstName = new TextField();
    subroot1.getChildren().addAll(t1, patientFirstName);
    ...
    b1.setOnAction((EventHandler<ActionEvent>) new EventHandler<ActionEvent>() {
      public void handle(ActionEvent ae) {
        String patientNameStr = "%" + patientFirstName.getText().trim() + "%";
        String SELECT_QUERY = "SELECT C.HR_ID, C.DATE_OF_CONSULT, P.FNAME, P.LNAME, H.SYMPTOMS, C.P_ID FROM CONSULTATIONS C, HEALTH_RECORDS H,PATIENTS P WHERE P.FNAME LIKE ? AND P.PATIENT_ID = C.P_ID AND C.MLICENSE = ? AND C.HR_ID = H.RECORD_NO;";
        try (PreparedStatement preparedStatement = JDBCService.getInstance().getConnection()
            .prepareStatement(SELECT_QUERY);) {
          preparedStatement.setString(1, patientNameStr);
          preparedStatement.setString(2, DoctorProvider.getInstance().getDoctor().getMLicense());
  ...
  private void tableInit() {
    ...
    patientName.setCellValueFactory(new PropertyValueFactory<Consultations, String>("patientName"));
          ...
          private final Button btn = new Button("View");
          {
            btn.setOnAction((ActionEvent event) -> {
              ...
              try (PreparedStatement preparedStatement = JDBCService.getInstance().getConnection()
                ...
                DViewHealthRecord dvhr = new DViewHealthRecord(newRecord, data.getPatientName(), data.getPID());
                NavigationService.getInstance().pushScreen(dvhr.display());
              } catch (SQLException e) {
                System.out.println("Exception caught" + e.toString());
              }
              System.out.println("selectedData: " + data.getHealthRecordNumber() + data.getPatientName());
            });
          }
    ...
    String SELECT_QUERY = "SELECT C.HR_ID, C.DATE_OF_CONSULT, P.FNAME, P.LNAME, H.SYMPTOMS, C.P_ID FROM CONSULTATIONS C, HEALTH_RECORDS H,PATIENTS P WHERE P.PATIENT_ID = C.P_ID AND C.MLICENSE = ? AND C.HR_ID = H.RECORD_NO;";
    try (PreparedStatement preparedStatement = JDBCService.getInstance().getConnection()
        .prepareStatement(SELECT_QUERY);) {
      preparedStatement.setString(1, DoctorProvider.getInstance().getDoctor().getMLicense());
      ResultSet rs = preparedStatement.executeQuery();
      data.clear();
      while (rs.next()) {
        System.out.println(rs.getString(3) + " " + rs.getString(4) + rs.getDate(2).toString() + rs.getString(5)
  ...
  public static class Consultations {
    ...
    Consultations(String hr_id, String dateOfConsultation, String patientName, String symptoms, int p_id) {
